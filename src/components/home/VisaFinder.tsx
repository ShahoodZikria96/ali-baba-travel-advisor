        ...
        <RevealGroup className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {popularDestinations.map((country) => (
            <RevealItem key={country.slug}>
              <TiltCard strength={7} className="rounded-[var(--radius-md)]">
                <Link
                  href={`/visas/${country.slug}`}
                  className="tilt-card-inner group block overflow-hidden rounded-[var(--radius-md)] border border-border bg-surface"
                >
