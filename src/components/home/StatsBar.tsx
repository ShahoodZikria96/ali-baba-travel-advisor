  ...
  return (
    <section className="relative overflow-hidden bg-charcoal">
      ...
      <RevealGroup className="relative">
        <Container className="grid grid-cols-2 gap-6 py-10 sm:grid-cols-4 lg:py-12">
          {stats.map(({ icon: Icon, value, label }) => (
            <RevealItem key={label} className="flex flex-col items-center gap-2 text-center sm:flex-row sm:items-start sm:text-left">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white/10 text-primary-light">
                <Icon size={20} />
              </span>
              <div>
                <p className="font-heading text-2xl font-extrabold text-white sm:text-[1.6rem]">
                  <CountUp value={value} />
                </p>
