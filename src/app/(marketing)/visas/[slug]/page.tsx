...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: rich?.metaTitle ?? `${basic.name} Visa from Pakistan`,
    description: rich?.metaDescription ?? basic.description,
  };
}
  ...
  return (
    <>
      ...
      <PageHero
        ...
        description={rich?.intro ?? basic.description}
      >
        <div className="mt-3 flex items-center gap-3">
          {basic.flagImage && (
            <Image src={basic.flagImage} alt={`${basic.name} flag`} width={48} height={30} className="h-7 w-11 rounded-sm object-cover" />
          )}
        </div>
      ...
      {rich ? (
        <Container className="grid grid-cols-1 gap-12 py-14 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <p className="mt-3 text-sm leading-relaxed text-text-muted">{rich.financialNote}</p>

            <h2 className="mt-10 font-heading text-xl font-bold text-charcoal">Processing Information</h2>
            <p className="mt-3 text-sm leading-relaxed text-text-muted">{rich.processingTime}</p>
            ...
            <ul className="mt-4 space-y-2 text-sm leading-relaxed text-text-muted">
              {rich.refusalReasons.map((reason) => (
          ...
          <div className="h-fit rounded-[var(--radius-lg)] border border-border bg-surface p-6 lg:sticky lg:top-24">
            <h2 className="font-heading text-lg font-bold text-charcoal">Get Your {basic.name} Visa Assessed</h2>
            <p className="mt-1.5 text-sm text-text-muted">Tell us about your travel plan and we&rsquo;ll get back to you.</p>
            <div className="mt-5">
              <VisaAssessmentForm defaultCountry={basic.name} />
            </div>
        ...
        <Container className="py-14">
          <p className="max-w-2xl text-sm leading-relaxed text-text-muted">
            {basic.description} Contact our consultants for a full assessment of {basic.name} visa requirements,
